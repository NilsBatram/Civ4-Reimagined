...
void CvGame::setInitialItems()
{
	...
	if (isGameMultiPlayer())
	{
		...
		double dHandicap = 0.5 + (double)iTotal/(double)iHumanPlayers;
		if (iHumanPlayers > 0)
			setHandicapType((HandicapTypes)((int)dHandicap)); // Civ4 Reimagined: Rounding
		else
...
void CvGame::reset(HandicapTypes eHandicap, bool bConstructorCall)
{
	...
	//Civ4 Reimagined
	m_bNukesValid = true;
	m_bIdeologiesEnabled = false; // Civ4 Reimagined
					...
					if (pLoopPlot != NULL)
					{
						if (pLoopPlot->getFeatureType() != NO_FEATURE)
						{
							if ((GC.getFeatureInfo(pLoopPlot->getFeatureType()).getYieldChange(YIELD_FOOD) <= 0) &&
								...
								(pLoopPlot->getBonusType() == NO_BONUS)) // Civ4 Reimagined
							{
								if (gMapLogLevel > 0)
									logBBAI("    Removing %S for player %d.", GC.getFeatureInfo(pLoopPlot->getFeatureType()).getDescription(), iI);
                                        ...
                                        for (iK = 0; iK < GC.getNumTerrainInfos(); iK++)
                                        {
                                            if (!(GC.getTerrainInfo((TerrainTypes)iK).isWater()))
                                            {
                                                    ...
                                                    (GC.getTerrainInfo((TerrainTypes)iK).getYield(YIELD_FOOD) + GC.getTerrainInfo((TerrainTypes)iK).getYield(YIELD_PRODUCTION)) == iTargetTotal)
                                                {
                                                    ...
                                                    {
														// Civ4 Reimagined
														if ((pLoopPlot->getBonusType(GET_PLAYER((PlayerTypes)iI).getTeam()) == NO_BONUS) || GC.getBonusInfo(pLoopPlot->getBonusType(GET_PLAYER((PlayerTypes)iI).getTeam())).isTerrain(iK))
														{
...
void CvGame::normalizeAddExtras()
{
	...
	int iMaxValue = GC.getDefineINT("MAX_STARTING_POS_VALUE");
	
	// Civ4 Reimagined
	BonusTypes BONUS_IRON = (BonusTypes)GC.getInfoTypeForString("BONUS_IRON");
	BonusTypes BONUS_COPPER = (BonusTypes)GC.getInfoTypeForString("BONUS_COPPER");
	BonusTypes BONUS_OIL = (BonusTypes)GC.getInfoTypeForString("BONUS_OIL");
	...
	// Civ4 Reimagined: TargetValue closer to BestValue
	int iTargetValue = (std::min(iBestValue, iMaxValue) * 5) / 6;
	...
	for (int iI = 0; iI < MAX_CIV_PLAYERS; iI++)
	{
		...
		int aiShuffle[NUM_CITY_PLOTS];
		shuffleArray(aiShuffle, NUM_CITY_PLOTS, getMapRand());
		bool bIsCoastal = pStartingPlot->isCoastalLand(GC.getMIN_WATER_SIZE_FOR_OCEAN());

		/* Removed by Civ4 Reimagined: Features don't really improve starting positions
		...
		*/
		...
		// Civ4 Reimagined: Moved this code up
		int iHillsCount = 0;
		...
		for (int iJ = 0; iJ < NUM_CITY_PLOTS; iJ++)
		{
			...
			if (pLoopPlot->getBonusType() == eUniqueBonus || eUniqueBonus == NO_BONUS || eUniqueBonus == BONUS_IRON || eUniqueBonus == BONUS_COPPER || eUniqueBonus == BONUS_OIL)
			{
				bHasUniqueBonus = true;
				break;
			}
		}
		...
		{
			if (kLoopPlayer.AI_foundValue(pStartingPlot->getX_INLINE(), pStartingPlot->getY_INLINE(), -1, true) > iMaxValue)
			{
				for (int iJ = 0; iJ < NUM_CITY_PLOTS; iJ++)
				{
					CvPlot* pLoopPlot = plotCity(pStartingPlot->getX_INLINE(), pStartingPlot->getY_INLINE(), aiShuffle[iJ]);
			...
			else
			{
				...
				for (int iJ = 0; iJ < NUM_CITY_PLOTS; iJ++)
				{
					...
					if ((pLoopPlot != NULL) && (pLoopPlot != pStartingPlot))
					{
						if (getSorenRandNum(((bLandBias && pLoopPlot->isWater()) ? 1 : 0), "Placing Bonuses") == 0)
						{
							if (kLoopPlayer.AI_foundValue(pStartingPlot->getX_INLINE(), pStartingPlot->getY_INLINE(), -1, true) >= iTargetValue)
							{
								if (gMapLogLevel > 0)
									logBBAI("    Player %d doesn't need any more bonuses to reach target value.", iI); // K-Mod
								break;
							}
							...
							if ((pLoopPlot != pStartingPlot)
								...
								&& (!(pLoopPlot->isWater()) || bIsCoastal)) // Civ4 Reimagined
							{
								for (int iPass = 0; iPass < 3; iPass++)
								{
									logBBAI("    iPass: %d", iPass);
									if (pLoopPlot->getBonusType() == NO_BONUS)
									{
										for (int iK = 0; iK < GC.getNumBonusInfos(); iK++)
										{
											if (GC.getBonusInfo((BonusTypes)iK).isNormalize() || ((BonusTypes)iK == eUniqueBonus && !bHasUniqueBonus))
											{
												if (iPass >= 1 || bHasUniqueBonus || (BonusTypes)iK == eUniqueBonus)
												{
			...
			{
				bool bRemovedBonus = false;
				shuffleArray(aiShuffle, NUM_CITY_PLOTS, getMapRand());
				for (int iPass = 0; iPass < 2 && !bRemovedBonus; iPass++)
				{
					for (int iJ = 0; iJ < NUM_CITY_PLOTS; iJ++)
					{
						CvPlot* pLoopPlot = plotCity(pStartingPlot->getX_INLINE(), pStartingPlot->getY_INLINE(), aiShuffle[iJ]);
						...
						if ((pLoopPlot != NULL) && (pLoopPlot != pStartingPlot) && pLoopPlot->getBonusType() != NO_BONUS)
						{
							if ((iPass > 0 || GC.getBonusInfo(pLoopPlot->getBonusType()).getHealth() <= 0) && pLoopPlot->getBonusType() != BONUS_IRON && pLoopPlot->getBonusType() != BONUS_COPPER && pLoopPlot->getBonusType() != BONUS_OIL)
							{
								...
								pLoopPlot->setBonusType(NO_BONUS);
								bRemovedBonus = true;
								//todo: remove bad terrain/features?
								break;
							}
						}
					}
				}
				for (int iPass = 0; iPass < 2 && !bHasUniqueBonus; iPass++)
				{
					for (int iJ = 0; iJ < NUM_CITY_PLOTS; iJ++)
					{
						CvPlot* pLoopPlot = plotCity(pStartingPlot->getX_INLINE(), pStartingPlot->getY_INLINE(), aiShuffle[iJ]);
						...
						if ((pLoopPlot != NULL) && (pLoopPlot != pStartingPlot) && (pLoopPlot->getBonusType() == NO_BONUS))
						{
							...
							{
								if (pLoopPlot->canHaveBonus(eUniqueBonus, bIgnoreLatitude))
								{
									if (gMapLogLevel > 0)
										logBBAI("    Adding %S for player %d for unique unit.", GC.getBonusInfo(eUniqueBonus).getDescription(), iI);
									pLoopPlot->setBonusType(eUniqueBonus);
									bHasUniqueBonus = true;
									break;
								}
							}
			...
			// Civ4 Reimagined: Still no bonus?
			if (!bHasUniqueBonus)
			{
				shuffleArray(aiShuffle, NUM_CITY_PLOTS, getMapRand());
				for (int iJ = 0; iJ < NUM_CITY_PLOTS; iJ++)
				{
					...
					{
						if ((GC.getBonusInfo(eUniqueBonus).isFlatlands()) && pLoopPlot->isHills())
						{
							continue;
						}
						if (gMapLogLevel > 0)
							logBBAI("    Adding %S and terrain for player %d for unique unit.", GC.getBonusInfo(eUniqueBonus).getDescription(), iI);
						
						if (pLoopPlot->getFeatureType() != NO_FEATURE)
						{
							if (!(GC.getBonusInfo(eUniqueBonus).isFeature(pLoopPlot->getFeatureType())))
							{
								pLoopPlot->setFeatureType(NO_FEATURE);
								
								if (!(GC.getBonusInfo(eUniqueBonus).isTerrain(pLoopPlot->getTerrainType())))
								{
									for (int iK = 0; iK < GC.getNumTerrainInfos(); iK++)
									{
										if ((GC.getBonusInfo(eUniqueBonus).isTerrain((TerrainTypes)iK)))
										{
											pLoopPlot->setTerrainType((TerrainTypes)iK);
											break;
										}
							...
							else if (!(GC.getBonusInfo(eUniqueBonus).isFeatureTerrain(pLoopPlot->getTerrainType())))
							{
								for (int iK = 0; iK < GC.getNumTerrainInfos(); iK++)
								{
									if ((GC.getBonusInfo(eUniqueBonus).isFeatureTerrain((TerrainTypes)iK)))
									{
										pLoopPlot->setTerrainType((TerrainTypes)iK);
										break;
									}
						...
						} else if (!(GC.getBonusInfo(eUniqueBonus).isTerrain(pLoopPlot->getTerrainType())))
						{
							for (int iK = 0; iK < GC.getNumTerrainInfos(); iK++)
							{
								if ((GC.getBonusInfo(eUniqueBonus).isTerrain((TerrainTypes)iK)))
						...
						if ((GC.getBonusInfo(eUniqueBonus).isHills()) && !pLoopPlot->isHills())
						{
							pLoopPlot->setPlotType(PLOT_HILLS, false, true);
						}
						pLoopPlot->setBonusType(eUniqueBonus);
...
// Civ4 Reimagined
double CvGame::getCurrentEraAverage() const
{
	int iEra;
	int iCount;
	int iI;
	...
	if (iCount > 0)
	{
		return ((double)iEra / (double)iCount);
	}
					...
					for (iI = 0; iI < MAX_PLAYERS; iI++)
					{
						if (GET_PLAYER((PlayerTypes)iI).isAlive())
						{
							...
							else
							{
								...
								gDLL->getInterfaceIFace()->addHumanMessage(((PlayerTypes)iI), false, GC.getDefineINT("EVENT_MESSAGE_TIME_LONG"), szBuffer, GC.getReligionInfo(eIndex).getSound(), MESSAGE_TYPE_MAJOR_EVENT, GC.getReligionInfo(eIndex).getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
							}
						}
					}
				}
...
void CvGame::createBarbarianUnits()
{
	...
	int iLoop;
	int iI, iJ;
	int iBarbsInWasteland; // Civ4 Reimagined
			...
			if (iDivisor > 0)
			{
				...
				if (iNeededBarbs > 0)
				{
					...
					for (iI = 0; iI < iNeededBarbs; iI++)
					{
						...
						{
							pPlot = GC.getMapINLINE().syncRandPlot((RANDPLOT_NOT_VISIBLE_TO_CIV | RANDPLOT_ADJACENT_LAND | RANDPLOT_PASSIBLE | RANDPLOT_NEEDS_FOOD), pLoopArea->getID(), GC.getDefineINT("MIN_BARBARIAN_STARTING_DISTANCE"));
						}
...
void CvGame::read(FDataStreamBase* pStream)
{
	...
	pStream->Read(&m_bIdeologiesEnabled); // Civ4 Reimagined
...
void CvGame::write(FDataStreamBase* pStream)
{
	...
	pStream->Write(m_bIdeologiesEnabled); // Civ4 Reimagined
...
bool CvGame::isCompetingCorporation(CorporationTypes eCorporation1, CorporationTypes eCorporation2) const
{
	...
	// Civ4 Reimagined: 
	if (GC.getCorporationInfo(eCorporation1).isSlaves() && GC.getCorporationInfo(eCorporation2).isSlaves())
	{
		return true;
	}
